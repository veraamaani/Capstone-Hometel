...
<head>
    ...
    <title>CRMC Hometel Management | Confirmed Reservation</title>
    <!-- Bootstrap CSS -->
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js" integrity="sha384-w76AqPfDkMBDXo30jS1Sgez6pr3x5MlQ1ZAGC+nuZB+EYdgRZgiwxhTBTkF7CXvN" crossorigin="anonymous"></script>
  ...
  <h2 class="noselect">Confirmed Reservation Page</h2>
  <hr style="border-color: black;"/>
  <div class="container-fluid">
  <p style="font-size:17px;"class="noselect">
    This page displays all confirmed reservation from the guests. Click 
    <button style="height:36px;border-radius:30px;"class="btn btn-success">View Details</button> to see the entire 
  ...
  <div class="col-sm-12"><!-- Div for Table (Start) -->
      <table class="table table-striped table-bordered" id="reservationTable">
        ...
        <tbody>
          <?php 
            $sqlQuery = " SELECT * FROM reservation WHERE status ='Confirmed' ORDER BY id DESC ";
            $res = $link->query($sqlQuery);
            while($row = mysqli_fetch_object($res)){
          ?>
          <tr>
            <td>
              Name: <?php echo $row->guest_name; ?>
            </td>
            <td class="text-center noselect">
              <button style="height:36px;border-radius:30px;" data-id='<?php echo $row->id; ?>' class="reservationDetails btn btn-success">View Details</button>
            </td>
          </tr>
          <?php } ?>
        </tbody>
  ...
  <div class="modal fade" id="viewConfirmedReservationModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1">
    <div class="modal-dialog">
      <div class="modal-content">
        <!--
        ***** Modal content provided by x_viewConfirmedReservation.php *****
        -->
        </div>
      </div>
    </div>
...
<script type='text/javascript'>
  ...
  // VIEW CONFIRMED RESERVATION Modal
  $(document).ready(function(){
    $('.reservationDetails').click(function(){
      var reservation_id = $(this).data('id');
      $.ajax
      ({
        url: 'x_viewConfirmedReservation.php',
        type: 'post',
        data: {id: reservation_id},
        success: function(response){ 
          $('.modal-content').html(response); 
          $('#viewConfirmedReservationModal').modal('show'); 
        }
      });
    });
  });
